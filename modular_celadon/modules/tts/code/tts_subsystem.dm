...
/datum/controller/subsystem/ttsSilero/stat_entry(msg)
	msg += "tRPS:[tts_trps] "
	msg += "rRPS:[tts_rrps] "
	msg += "RPS:[tts_rps] "
...
/datum/controller/subsystem/ttsSilero/fire()
...
/datum/controller/subsystem/ttsSilero/Recover()
	is_enabled = SSttsSilero.is_enabled
	tts_wanted = SSttsSilero.tts_wanted
	tts_request_failed = SSttsSilero.tts_request_failed
	tts_request_succeeded = SSttsSilero.tts_request_succeeded
	tts_reused = SSttsSilero.tts_reused

/datum/controller/subsystem/ttsSilero/proc/queue_request(text, datum/tts_seed/seed, datum/callback/proc_callback)
	if(LAZYLEN(tts_requests_queue) > tts_requests_queue_limit)
		is_enabled = FALSE
		to_chat(world, span_announce("SERVER: очередь запросов превысила лимит, подсистема SSttsSilero принудительно отключена!"))
		return FALSE
...
/datum/controller/subsystem/ttsSilero/proc/get_tts(atom/speaker, mob/listener, message, seed_name, is_local = TRUE, effect = SOUND_EFFECT_NONE, traits = TTS_TRAIT_RATE_FASTER, preSFX = null, postSFX = null)
	if(!is_enabled)
		return
	if(!message)
		return
...
/datum/controller/subsystem/ttsSilero/proc/get_tts_callback(atom/speaker, mob/listener, filename, datum/tts_seed/seed, is_local, effect, preSFX, postSFX, datum/http_response/response)
	var/datum/tts_provider/provider = seed.provider
...
/datum/controller/subsystem/ttsSilero/proc/play_tts(atom/speaker, mob/listener, filename, is_local = TRUE, effect = SOUND_EFFECT_NONE, preSFX = null, postSFX = null)
	if(isnull(listener) || !listener.client)
		return
	...
	// Celadon REMOVAL var/turf/turf_source = get_turf(speaker)
	...
	// Celadon REMOVAL TODO: fix once TTS used or remove entirely all subsystems listener.playsound_local(turf_source, output, volume, sound_to_use = output, channel = channel, wait = TRUE)
...
/datum/controller/subsystem/ttsSilero/proc/play_sfx(mob/listener, sfx, channel, volume, environment)
...
/datum/controller/subsystem/ttsSilero/proc/cleanup_tts_file(filename)
	fdel(filename)

/datum/controller/subsystem/ttsSilero/proc/get_available_seeds(owner)
...
/datum/controller/subsystem/ttsSilero/proc/get_random_seed(owner)
	return pick(get_available_seeds(owner))

/datum/controller/subsystem/ttsSilero/proc/sanitize_tts_input(message)
	var/hash
	if(sanitized_messages_caching)
...
/proc/tts_cast(atom/speaker, mob/listener, message, seed_name, is_local = TRUE, effect = SOUND_EFFECT_NONE, traits = TTS_TRAIT_RATE_FASTER, preSFX = null, postSFX = null)
	SSttsSilero.get_tts(speaker, listener, message, seed_name, is_local, effect, traits, preSFX, postSFX)
