...
/obj/item/paper/ui_act(action, params, datum/tgui/ui)
	...
	switch(action)
		...
		if("add_text")
			...
			var/parsed_input = parsemarkdown(paper_input, user) // Celadon ADDITION
			add_raw_text(parsed_input, writing_implement_data["font"], writing_implement_data["color"], writing_implement_data["use_bold"], check_rights_for(user?.client, R_FUN)) // Celadon EDIT, original: add_raw_text(paper_input, writing_implement_data["font"], writing_implement_data["color"], writing_implement_data["use_bold"], check_rights_for(user?.client, R_FUN))
