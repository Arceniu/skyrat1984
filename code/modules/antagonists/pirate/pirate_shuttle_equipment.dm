...
/obj/machinery/computer/piratepad_control/proc/pirate_export_loop(obj/machinery/piratepad/pad, dry_run = TRUE)
	...
	for(var/atom/movable/item_on_pad as anything in get_turf(pad))
		...
		for(var/mob/living/hidden as anything in hidden_mobs)
			...
			export_single_item(hidden, apply_elastic = FALSE, dry_run = dry_run, external_report = report, export_markets = export_markets_override) // Celadon EDIT, original: export_single_item(hidden, apply_elastic = FALSE, dry_run = dry_run, external_report = report, export_markets = list(EXPORT_MARKET_STATION, EXPORT_MARKET_PIRACY))
