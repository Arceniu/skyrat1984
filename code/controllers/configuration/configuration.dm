...
/datum/controller/configuration/proc/Load(_directory)
	...
	InitEntries()
	if (fexists("[directory]/config_preload.txt")) // Used for setting values such as use_server_config on server-side
		LoadEntries("config_preload.txt")
	if (CONFIG_GET(flag/use_server_config) && fexists("[directory]/config_server.txt"))
		log_config("Loading config_server.txt instead of config.txt")
		LoadEntries("config_server.txt")
	else
