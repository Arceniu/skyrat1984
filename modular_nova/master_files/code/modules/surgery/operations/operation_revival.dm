...
/datum/surgery_operation/basic/revival/on_failure(mob/living/patient, mob/living/surgeon, obj/item/tool, list/operation_args)
	if(HAS_TRAIT(patient, TRAIT_DNR))
		patient.visible_message(span_warning("...[patient.p_they()] lie[patient.p_s()] still, unaffected. Further attempts are futile, [patient.p_theyre()] gone.")) // SS1984 EDIT, original: patient.visible_message(span_warning("...[patient.p_they()] lie[patient.p_s()] still, unaffected. Further attempts are futile, patient.p_theyre() gone."))
